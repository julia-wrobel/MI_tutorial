...
The data is publicly available on Bioconductor as the package `VectraPolarisData`. You can install the `VectraPolarisData` from Bioconductor here:
...
* `membraine_intensities`: mean marker intensity in the cell membrane
...
Storing the data as a `SpatialExperiment` allows for analysis using tools that are built into the `SpatialExperiment` workflow.  However, I often find it useful to work with the data as a `data.frame` object instead.

Code below converts the ovarian cancer dataset from a `SpatialExperiment` object to a `data.frame`.
...
Plotting the cells for a single subject
...
For multiple subjects. Each slides has multiple tissue slices from a tumor microarray, and x and y locations correspond to position on the slide of a given sample. There is only one sample per subject in this dataset.
...
Load the non small cell lung carcinoma data and inspect the `SpatialExperiment` object. This dataset has 3-4 images per patient, representing different ROIs from a tissue slice.
...
cell_level_lung <- as_tibble(cbind(colData_df, 
  ...
  dplyr::rename(cd19 = cd19_opal_650,
                ...
                ck = ck_opal_570) %>%
  dplyr::select(cd14:sample_id, slide_id:tissue_category, cell_id,
                entire_cell_axis_ratio:entire_cell_area_square_microns)
...
Images from a single subject. This data is not from a TMA. Each image represents an ROI from a tissue slice. There are multiple ROIs for each subject, and each of the images below represents a different ROI for the same subject. 
...
Processed data are saved as `.rda` objects and can also be downloaded directly from the short course website.


```{r, eval = FALSE}
# save data sets
save(ovarian_df, file = here::here("DataCode", "ovarian.RDA"))
save(lung_df, file = here::here("DataCode", "lung.RDA"))

# load processed ovarian data
load(url("https://github.com/julia-wrobel/MI_tutorial/raw/main/DataCode/ovarian.RDA"))
