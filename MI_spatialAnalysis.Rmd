...
First create a spatstat`ppp` object. We will do spatial analysis on the the distribution of B-cells and macrophages, so subset to only these cell types.
...
Calculate Ripley's K for B cells, then B-cells and Macrophages.  
...
You can plot this object using spatstat to visualize the estimated K ($\hat{K}^{iso}$) compared to the K under CSR ($K^{pois}$).
